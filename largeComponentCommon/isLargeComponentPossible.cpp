...
namespace largeComponent
{
	bool isLargeComponentOn(const context::inputGraph& graph, const vertexState* state, std::size_t componentSize, isLargeComponentPossibleTemporaries& temporaries)
	{
		...
		{
			if(state[i].state & (FIXED_ON | UNFIXED_ON))
			{
		...
		for(std::size_t i = 0; i < nVertices; i++)
		{
			if(state[i].state & (FIXED_ON | UNFIXED_ON)) temporaries.table[temporaries.connectedComponents[i]]++;
		}
	...
	bool isLargeComponentPossible(const context::inputGraph& graph, const vertexState* state, std::size_t componentSize, isLargeComponentPossibleTemporaries& temporaries)
	{
		...
		for(std::size_t i = 0; i < nVertices; i++)
		{
			if(state[i].state & (FIXED_ON | UNFIXED_MASK))
			{
				temporaries.initialPoints.push_back((int)i);
				temporaries.colourVector[i] = Color::white();
			}
		...
		for(std::size_t i = 0; i < nVertices; i++)
		{
			if(state[i].state & (FIXED_ON | UNFIXED_MASK)) temporaries.table[temporaries.connectedComponents[i]]++;
		}
		return *std::max_element(temporaries.table.begin(), temporaries.table.end()) >= (int)componentSize;
	}
}
